...
import (
	...
	"github.com/zhangel/go-frame.git/3rdparty/grpc-gateway/httprule"
...
func (s *ServerMux) registerMethods(grpcServiceDesc grpc_server.ServiceDescList) error {
	...
	for _, sd := range grpcServiceDesc {
		if len(sd.RegisterOption.Alias) > 0 {
			headerAlias[sd.ServiceInfo.ServiceName] = append(headerAlias[sd.ServiceInfo.ServiceName], sd.RegisterOption.Alias...)
		}
...
func (s *ServerMux) makeHttpRuleRequest(r *http.Request) (func(ctx context.Context, w http.ResponseWriter), error) {
	if !strings.HasPrefix(utils.HttpPath(r), "/") {
		return nil, fmt.Errorf("http mux: makeHttpRuleRequest: not a valid path")
	}

	var verb string
	components := strings.Split(utils.HttpPath(r)[1:], "/")
	l := len(components)
	...
	CheckPathParam := func(_ *http.Request, meta interface{}, _ int) error {
