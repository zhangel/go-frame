...
func PeerTlsInfo(ctx context.Context) (*credentials.TLSInfo, error) {
	if tlsInfo, err := peerTlsInfoFromMeta(ctx, nil); err == nil {
		...
		log.Tracef("PeerTlsInfoFromMeta got nothing[%v], try to get tlsInfo from Peer", err)
		if tlsInfo, err := peerTlsInfoFromPeer(ctx); err == nil {
			return tlsInfo, nil
		} else {
			return nil, err
		}
...
func MarshalTlsInfo(tlsInfo *credentials.TLSInfo, key *rsa.PrivateKey, simplify bool) (string, error) {
	if tlsInfo == nil {
		return "", fmt.Errorf("marshal tls info failed, tlsInfo is nil")
	}

	if key == nil {
		return "", fmt.Errorf("marshal tls info failed, rsaPrivateKey is nil")
	}
	...
	if tlsInfoMeta, ok := tlsInfoToMetaMap.Load(tlsInfoKey); ok {
		return tlsInfoMeta.(string), nil
	}
	...
	copyCertificateInfo := func(src, dest *x509.Certificate) {
		if !simplify {
			...
			dest.Subject.CommonName = src.Subject.CommonName
		}
	...
	if err := encoder.Encode(&tlsInfoForward); err != nil {
		return "", fmt.Errorf("gob encode tls info failed, err = %v", err)
	}
	...
	if _, err := gzipWriter.Write(gobTlsInfoBuf.Bytes()); err != nil {
		return "", fmt.Errorf("gzip encode tls info failed, err = %v", err)
	}
	...
	if err != nil {
		return "", fmt.Errorf("create rsa-sha256 signature failed, err = %v", err)
	}
...
func PeerTlsInfoToMeta(ctx context.Context, tlsInfo *credentials.TLSInfo, key *rsa.PrivateKey) (context.Context, error) {
	...
	tlsInfoMeta, err := MarshalTlsInfo(tlsInfo, key, false)
	if err != nil {
		return ctx, err
	}
	...
	return SetTlsInfoHeader(ctx, tlsInfoMeta)
}
