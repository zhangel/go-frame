...
func ContextWithReqId(ctx context.Context, reqId, pid, cid, rpc, prpc string) context.Context {
	if _, ok := ctx.Value(timestampCtx{}).(int64); !ok {
...
func ContextWithTracingInfo(ctx context.Context) context.Context {
	if _, ok := ctx.Value(timestampCtx{}).(int64); !ok {
		ctx = context.WithValue(ctx, timestampCtx{}, time.Now().UnixNano())
	}

	if _, ok := ctx.Value(tracingInfoCtx{}).(*TracingInfo); ok {
		return ctx
	} else {
...
func RequestIdTracingUnaryServerInterceptor(ctx context.Context, req interface{}, info *grpc.UnaryServerInfo, handler grpc.UnaryHandler) (interface{}, error) {
	...
	tracingInfo, _ := ctx.Value(tracingInfoCtx{}).(*TracingInfo)
	if tracingInfo == nil {
		if ti, ok := TracingInfoFromMetadata(ctx); ok {
			tracingInfo = ti
		} else {
			tracingInfo = ChildTracingInfo("S_"+info.FullMethod, &TracingInfo{})
		}
	}
...
func RequestIdTracingStreamingServerInterceptor(srv interface{}, ss grpc.ServerStream, info *grpc.StreamServerInfo, handler grpc.StreamHandler) error {
	ctx := context.WithValue(ss.Context(), timestampCtx{}, time.Now().UnixNano())
...
func HttpServerReqIdHandler(handler http.Handler) http.Handler {
	return http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		var reqId string
		if reqIdHeader := r.Header[tagHttpRid]; len(reqIdHeader) == 0 || reqIdHeader[0] == "" {
			reqId = "S_" + generateReqId()
		} else {
			reqId = reqIdHeader[0]
		}

		ctx := context.WithValue(r.Context(), timestampCtx{}, time.Now().UnixNano())
		callId := reqId
		if callIdHeader := r.Header[tagHttpCid]; len(callIdHeader) > 0 && callIdHeader[0] != "" {
			callId = callIdHeader[0]
		}

		rpc := "/" + r.Method + utils.HttpPath(r)
		if rpcHeader := r.Header[tagHttpRpc]; len(rpcHeader) > 0 && rpcHeader[0] != "" {
			rpc = rpcHeader[0]
		}
...
func tracingInfoToLanxinCallstack2(ctx context.Context, tracingInfo *TracingInfo, md metadata.MD, ts int64) {
	...
	if !ok || getMdItem(mdIncoming, lanxinCallstack2) == "" {
		return
	}
...
func ChildTracingInfo(method string, tracingInfo *TracingInfo) *TracingInfo {
	...
	childTracingInfo.cid = generateReqId()
