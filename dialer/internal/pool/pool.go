...
var (
	...
	refTable map[string]map[string]struct{} = map[string]map[string]struct{}{}
...
func WithPool(dialer DialerFn, callOpts *option.CallOptions) (pool *ccPool, err error) {
	...
	defer func() {
		if callOpts.CloseRegister == nil {
			return
		}

		id, register := callOpts.CloseRegister()
		if register == nil {
			return
		}

		mutex.RLock()
		if idSet, ok := refTable[poolId]; ok {
			if _, ok := idSet[id]; ok {
				mutex.RUnlock()
				return
			}
		...
		idSet, ok := refTable[poolId]
		if !ok {
			idSet = map[string]struct{}{}
			refTable[poolId] = idSet
		}
		...
		register(func(id string) {
			mutex.Lock()
			refSet, ok := refTable[poolId]
			if !ok {
				log.Warnf("ccClose: no %q found in ccPool, cc.Close() ignored!", poolId)
				mutex.Unlock()
				return
			}
			...
			delete(refSet, id)
			if len(refSet) > 0 {
				log.Infof("ccClose: pool = %q has others ref [%v], ignore Close() from %q", poolId, refSet, id)
				mutex.Unlock()
				return
			}
			mutex.Unlock()

			log.Infof("ccClose: pool = %q removed from ccPool, cc in pool closed", poolId)
			close(poolId)
		})
	}()
...
func Clear() {
	...
	log.Info("ccPool cleared")
}

func close(poolId string) {
	mutex.Lock()
	defer mutex.Unlock()
	...
	result, err, _ := s.group.Do(strconv.Itoa(int(idx)), func() (result interface{}, err error) {
		...
		if ok && cc != nil && cc != nilCC {
			if err := s.checkConnState(waitContext, cc, beginTime); err == nil {
				...
				if s.lastFail[idx].Add(redialInterval).Before(time.Now()) || cc.GetState() == connectivity.Shutdown {
